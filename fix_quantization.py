...
# 修复后的 quantization_config.py
fixed_quantization_config = '''# quantization_config.py
...
def update_mast3r_utils(engine_path: str):
    ...
    update_code = f'''
# 在mast3r_utils.py中添加以下代码

def load_quantized_mast3r(engine_path="{engine_path}", device="cuda"):
    """加载量化版本的MASt3R模型"""
    try:
        from quantization_config import QuantizedMASt3RModel, QuantizationConfig
        ...
        print(f"成功加载量化模型: {{engine_path}}")
        return quantized_model
        
    except Exception as e:
        print(f"加载量化模型失败，回退到原始模型: {{e}}")
        return load_mast3r(device=device)

# 修改原有的load_mast3r函数来支持量化
def load_mast3r(path=None, device="cuda", use_quantized=False, engine_path=None):
    ...
    if use_quantized and engine_path and Path(engine_path).exists():
        return load_quantized_mast3r(engine_path, device)
...
'''
    ...
    print("请将以下代码添加到 mast3r_slam/mast3r_utils.py 中:")
    print(update_code)
...
if __name__ == "__main__":
    ...
    # 保存配置
    config.save_config("quantization_config.yaml")
    ...
    if quantized_model and engine_path:
        ...
        print("请按照提示更新代码以使用量化模型")
        
        # 生成更新指令
        update_mast3r_utils(engine_path)
'''

def fix_files():
    """修复量化相关文件"""
    
    # 保存修复后的 quantization_config.py
    with open("quantization_config.py", "w") as f:
        f.write(fixed_quantization_config)
    
    print("已修复 quantization_config.py")
    print("现在可以重新运行量化命令:")
    print("python run_quantization.py --precision int8")
